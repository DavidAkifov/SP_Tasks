...
int main(int argc, char *argv[])
{
    ...
    if (sema == SEM_FAILED)
    {
        perror("[TASK_TWO] sem_open/TaskTwo");
        exit(EXIT_FAILURE);
    }
    // ------------------------------------------------
    ...
    printf("\n\x1b[%d;%dm[TASK_TWO] Task 2 has started!\x1b[0m\n", text_color, back_color);
    ...
    {
        // Child process
        printf("\n[TASK_TWO] This is the child process (PID: %d)\n", getpid());
        exec_task_5();
    } 
    ...
    {
        sem_wait(sema);
        printf("\n[TASK_TWO] Task 2 int input: ");
        scanf("%d", &input);
        printf("\x1b[%d;%dmNumber entered:%d!\x1b[0m\n", text_color, back_color, input);
        sem_post(sema);

        // Send the number to Task 5 
        SerializedData serializedData;
        // Serialize and write integer data to the named pipe
        serializedData.type = INTEGER;
        serializedData.data.integerData = input;
    ...
    close(fd);
    sem_close(sema);
    printf("\nShutting down taks 2\n");
    exit(EXIT_SUCCESS);
}

void sigterm_handler() 
{
    printf("\n[TASK_TWO] Received SIGTERM signal. Terminating child process gracefully...\n");
    printf("\n[TASK_TWO] Will kill task 5\n");
    kill(pid, SIGTERM);  
    running = 0;
}
