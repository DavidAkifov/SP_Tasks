...
int main(int argc, char *argv[])
{
    ...
    if (sema == SEM_FAILED)
    {
        perror("[TASK_FOUR] sem_open/TaskFour");
        exit(EXIT_FAILURE);
    }
    // ------------------------------------------------
    ...
    printf("\n\x1b[%d;%dm[TASK_FOUR] Task 4 has started!\x1b[0m\n", text_color, back_color);
    ...
    {
        // Child process
        printf("\n[TASK_FOUR] This is the child process (PID: %d)\n", getpid());
        exec_task_5();
    } 
    ...
    {
        sem_wait(sema);
        printf("\n[TASK_FOUR] Task 4 string input: ");
        // scanf("%f", &input);
        fgets(input, sizeof(input), stdin);
        printf("\n\x1b[%d;%dm[TASK_FOUR] Number entered:%s!\x1b[0m\n", text_color, back_color, input);
        sem_post(sema);
        ...
        strcpy(serializedData.data.stringData, input);
    ...
    close(fd);
    sem_close(sema);
    printf("\n[TASK_FOUR] Shutting down taks 4\n");
    exit(EXIT_SUCCESS);
}

void sigterm_handler() 
{
    printf("\n[TASK_FOUR] Received SIGTERM signal. Terminating child process gracefully...\n");
    printf("\n[TASK_FOUR] Will kill task 5\n");
    kill(pid, SIGTERM);  
    running = 0;
}
