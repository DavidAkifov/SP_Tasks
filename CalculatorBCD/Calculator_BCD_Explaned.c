...
int findIntLength(int num) { // Спомагателна фиункция за намиране на дължината на инта
    ...
    if (num < 0) { //ако цислото е отрицателно, връщаме 0
        return 0;
    }
...
void print_bcd_in_binary(uint64_t number, uint8_t flag) 
{
    ...
    for (int i = 60; i >= 0; i -= 4) {
        ...
        if (bit > 0 || first_one_found || i == 0) { //проверяваме дали не е 1, или вече не е минат първия бит, или I не е = 0 (за да принтне дори резултат 0)
...
uint64_t subtract(int first_number, int second_number) {
    ...
    } else if (!first_number) { // Хендълваме вход 0 на първо число
